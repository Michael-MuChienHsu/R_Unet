...
class recurrent_network(nn.Sequential):
    def __init__(self, hidden_layer1, hidden_layer2,use_buffer = False):
        ...
        self.hidden = (hidden_layer1, hidden_layer2)
    ...
    def forward(self, x):
        if self.use_buffer == False:
            ...
            # after each step, hidden contains the hidden state.
                self.out, self.hidden = self.rnn(i, self.hidden)
            ...
            for i in x:
                ...
                # after each step, hidden contains the hidden state.
                self.out, self.hidden = self.rnn(i, self.hidden)
                self.our_put_buffer.append(self.out)
...
class unet(nn.Module):
    def __init__(self, tot_frame_num = 100, length = 6):
        ...
        self.lstm_buf = []
        # (hidden_layer num, second_dim, output channel)

        self.hidden11 = torch.zeros(1, 16, 16).cuda()
        ...
        self.up1 = Up_Layer(1024, 512).cuda()
    ...
    def forward(self, x):
        ...
        if len( self.lstm_buf ) > 1 :
            lstm_output_buffer = self.rnn(self.lstm_buf)
            lstm_output = self.rnn2( lstm_output_buffer )
